...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        Boolean isAnyStateGreaterThan21000Elevation = states.stream().anyMatch(s -> s.getHighestElevationInFeet() == 2100);
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(l -> l.getStateAnthem() == "");
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(l -> l.getStateMotto() != null);
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().collect(Collectors.averagingDouble(s ->s.getCapital().getAverageYearlyPrecipitationInInches()));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().
                collect(Collectors.groupingBy(s -> s.getCapital().getTimeZone(),
                        Collectors.counting()));
        ...
        List<String> allDistinctStateBirds = states.stream().map(s -> s.getStateBird()).distinct().collect(toList());
...
//        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().map(s -> s.getStateBird()).filter(l -> l.g).distinct().collect(toList());;
...
//        testResults.put("A24", StatesAndCapitalsCheck.adv24(allDistinctStateBirdsMinusMockingbirds));
        ...
        Long numberOfDistinctStateBirds =  states.stream().map(s -> s.getStateBird()).distinct().collect(counting());
        ...
        Integer maxStateElevation = states.stream().map(s -> s.getHighestElevationInFeet()).max(Comparator.naturalOrder()).orElseThrow();
